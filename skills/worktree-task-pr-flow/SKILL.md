...
9. Create the PR to `main`.
   ...
   - If GitHub Apps fails because of unavailable tools, unsupported operations, or integration permission errors such as `403 Resource not accessible by integration`, use `gh` as the fallback and record the fallback reason. Treat the PR operation as complete when `gh` succeeds.
...
- Do not move a task to `done` when known validation failures or blocked PR actions remain. A GitHub Apps failure is not a blocked PR action when the required PR creation, update, or comment was completed through `gh` and the fallback reason was recorded.
