...
3. Use `gh` only when the GitHub app lacks the needed operation, is unavailable, or fails with an integration-permission error such as `403 Resource not accessible by integration`.
4. When GitHub Apps fails and `gh` completes the same required PR operation, mark that PR operation complete and record the GitHub Apps failure plus `gh` fallback in the PR text, comments, work report, and final response where relevant.
5. If neither GitHub Apps nor `gh` can complete a required PR operation, report the task as blocked or partially complete. Do not mark the PR step complete.
...
3. Include:
   ...
   - work report path
4. Create or update the PR with GitHub Apps first. If GitHub Apps cannot perform the operation because of unavailable tools, unsupported operations, or integration permission errors such as `403 Resource not accessible by integration`, use `gh` as the fallback.
...
If a fallback was used, the PR creation or comment step is still complete when `gh` succeeds. The fallback reason must remain visible in the PR body or a PR comment, and in the work report.
