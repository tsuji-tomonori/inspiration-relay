...
- PR 作成・PR コメント投稿は GitHub Apps を優先する。`403 Resource not accessible by integration` など GitHub Apps の権限不足・未対応操作で失敗した場合は、失敗理由を PR 本文・コメント・作業レポートに記録したうえで `gh` にフォールバックしてよい。
- `gh` フォールバックで PR 作成・更新・コメント投稿が成功した場合、その GitHub 操作は完了扱いにする。GitHub Apps と `gh` の両方で必要な PR 操作を完了できない場合のみ blocked / partially complete として報告する。
